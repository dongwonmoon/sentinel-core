...
def mock_external_libs():
    ...
    """
    with patch(
        "src.embeddings.ollama.OllamaEmbeddings", MagicMock()
    ) as mock_ollama_emb, patch(
        "src.llms.ollama.ChatOllama", MagicMock()
    ) as mock_chat_ollama, patch(
        "src.store.pg_vector_store.create_async_engine", MagicMock()
    ) as mock_engine, patch(
        "src.factories.CrossEncoder", MagicMock()
    ) as mock_cross_encoder, patch(
        "src.factories.get_duckduckgo_search_tool"
    ) as mock_get_ddg_tool:

        # 각 Mock 객체가 spec에 정의된 인터페이스를 따르도록 설정
        mock_get_ddg_tool.return_value = create_autospec(
            BaseTool, instance=True
        )
...
def test_unsupported_type_factories():
    ...
    with pytest.raises(ValidationError):
        get_vector_store(
            Settings(VECTOR_STORE_TYPE="unsupported"), mock_embedding_model
        )
