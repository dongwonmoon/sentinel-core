...
class AgentNodes:
    """
    LangGraph 노드가 호출하는 실제 비즈니스 로직 모음.
    LLM, 벡터 스토어, 도구 등을 주입받아 각 단계가 필요한 컴포넌트를 직접 제어한다.
    그래프 자체는 선언적이지만, 각 노드가 어떤 부수효과를 내는지는 이 클래스에서 확인할 수 있다.
    """
    ...
    def __init__(
        ...
        # DB 접근이 필요한 노드(예: 장기 기억, 동적 도구 로드)를 위해, vector_store에서 DB 세션 팩토리를 가져옵니다.
        # 이 아키텍처는 vector_store가 DB 연결 정보를 중앙에서 관리하고, 다른 컴포넌트들이
        # 이를 공유하여 사용하는 '의존성 주입' 패턴을 따릅니다.
        # 이를 통해 DB 연결 설정을 한 곳에서 관리할 수 있어 유지보수성이 향상됩니다.
    ...
    async def build_hybrid_context(self, state: AgentState) -> Dict[str, Any]:
        ...
        async def get_short_term_memory() -> str:
            ...
            try:
                # AgentState에서 'chat_history'를 가져와 마지막 3개의 턴을 선택합니다.
                ...
                # 메모리 로드 중 오류가 발생하더라도 전체 에이전트 실행이 중단되지 않도록
                # 오류를 로깅만 하고 빈 문자열을 반환합니다.
        ...
        async def get_narrative_memory() -> str:
            ...
            try:
                ...
                # 대화가 너무 짧으면 요약의 의미가 없으므로 5턴 이상일 때만 실행합니다.
                if len(full_history) < 5:
                    return ""
                history_str = "\n".join(
                    [f"{msg['role']}: {msg['content']}" for msg in full_history]
                )
                # 요약 프롬프트에 전체 대화 기록과 현재 질문을 함께 제공하여
                # 질문과 관련된 맥락을 중심으로 요약하도록 유도합니다.
        ...
        async def get_long_term_memory() -> str:
            ...
            if not self.AsyncSessionLocal:
                return ""
            try:
                # 현재 질문을 임베딩하여 벡터 검색에 사용합니다.
                query_embedding = self.vector_store.embedding_model.embed_query(
                    state["question"]
                )
                # 'chat_turn_memory' 테이블에서 현재 세션과 사용자에 해당하는 과거 대화 중
                # 현재 질문 임베딩과 가장 유사한(거리가 가까운) 2개의 대화 턴을 검색합니다.
                sql_query = """
                ...
                async with self.AsyncSessionLocal() as session:
                    ...
                    # 벡터 거리가 0.8 미만인, 즉 충분히 관련성이 높은 대화만 필터링합니다.
                    relevant_turns = [
        ...
        async def get_available_dynamic_tools() -> List[DynamicTool]:
            ...
            if not self.AsyncSessionLocal:
                return []
            try:
                # 'registered_tools' 테이블에서 활성화된 도구 중,
                # 사용자의 'permission_groups' 중 하나라도 도구의 'permission_groups'와
                # 겹치는 도구를 모두 조회합니다. (&& 연산자: 배열 교차)
                sql_query = """
                ...
                async with self.AsyncSessionLocal() as session:
                    ...
                    # 조회 결과를 Pydantic 모델(DynamicTool) 리스트로 변환합니다.
        ...
        # 4가지 정보 수집 작업을 asyncio.gather를 사용해 병렬로 실행하여 전체 응답 시간을 단축합니다.
        ...
        # filter(None, ...)을 사용하여 비어있는 결과(예: 요약 실패)는 최종 컨텍스트에서 제외합니다.
    ...
    async def route_query(self, state: AgentState) -> Dict[str, Any]:
        ...
        try:
            # LLM의 응답이 JSON 형식으로 시작하는지 확인하여 동적 도구 호출 여부를 판단합니다.
            if response_content.startswith("{"):
                ...
                # LLM이 환각(hallucination)으로 존재하지 않는 도구를 생성할 수 있으므로,
                # 반환된 도구 이름이 실제 사용 가능한 동적 도구 목록에 있는지 반드시 확인합니다.
                tool_to_call = next(
            ...
            # LLM이 목록에 없는 도구를 반환할 경우를 대비해, 알려진 도구가 아니면 'None'으로 처리합니다.
            tool_choice = (
                tool_part
            ...
            # LLM의 출력이 예상된 형식(JSON 또는 '[LLM, TOOL]')이 아니어서 파싱에 실패할 경우,
            # 에이전트가 멈추지 않고 계속 진행하도록 안전한 기본값('도구 없음')으로 처리합니다.
            # 이는 LLM 기반 시스템의 안정성을 확보하는 데 매우 중요한 fallback 메커니즘입니다.
            logger.warning(
    ...
    async def run_dynamic_tool(self, state: AgentState) -> Dict[str, Any]:
        ...
        # route_query 노드에서 결정한 동적 도구 정보와 LLM이 생성한 입력값을 상태에서 가져옵니다.
        ...
        try:
            # 재사용 가능한 비동기 HTTP 클라이언트를 사용하여 지정된 엔드포인트에 POST 요청을 보냅니다.
            # (향후 이 부분에 OAuth2 등 기업 내부 시스템에 접근하기 위한 인증 헤더를 추가할 수 있습니다.)
            response = await http_client.post(endpoint_url, json=tool_input)
            response.raise_for_status()  # 4xx, 5xx 에러 코드 수신 시 예외를 발생시킵니다.

            result_data = response.json()
            # API 응답(JSON)을 문자열로 변환하여 다음 LLM 호출의 컨텍스트로 제공합니다.
            ...
            # API가 오류 응답을 반환한 경우, LLM이 오류를 이해하고 후속 조치를 취할 수 있도록
            # 상태 코드와 응답 본문을 포함한 명확한 오류 메시지를 생성합니다.
            ...
            # 네트워크 오류 등 예기치 않은 예외 발생 시에도 에이전트가 중단되지 않도록 처리합니다.
    ...
    async def run_rag_tool(self, state: AgentState) -> Dict[str, Any]:
        """
        [노드 2-A: RAG] 'Session KB'(세션 한정 임시 지식)만 검색합니다.
        """
        logger.debug("--- [Agent Node: Session RAG Tool] ---")
        question = state["question"]
        query_embedding = self.vector_store.embedding_model.embed_query(
            question
        )
        ...
        try:
            # 1. Session KB에서만 관련 문서를 검색합니다.
            # 이는 현재 대화의 맥락에 맞는 첨부 파일(임시 지식)에만 집중하기 위함입니다.
            session_docs = await self.vector_store.search_session_attachments(
                query_embedding=query_embedding,
                session_id=state["session_id"],
                k=5,  # 초기 검색 개수 (하드코딩, 향후 설정으로 분리 가능)
            )
            logger.debug(f"세션 RAG 검색 완료: {len(session_docs)}건")

            if not session_docs:
                logger.info(
                    "세션 RAG 검색 결과 없음 - 질문='%s'", question[:80]
                )
                # 도구 실행 실패를 상태에 기록하여, 라우터가 다음 번에 다른 도구를 선택하도록 유도합니다.
            ...
            # 2. Reranker를 사용하여 검색된 문서들의 순위를 재조정합니다.
            # 벡터 유사도만으로는 파악하기 힘든 의미적 관련성을 한 번 더 평가하여 정확도를 높입니다.
            reranked_docs = self.reranker.rerank(question, session_docs)

            # 3. 최종적으로 LLM에 전달할 상위 K개의 문서를 선택합니다.
            final_docs = reranked_docs[: state["top_k"]]
            logger.info(
                "세션 RAG 리랭킹 완료 - 원본 %d건, 최종 %d건 선택",
                len(session_docs),
                len(final_docs),
            )
            ...
            logger.error(f"세션 RAG 실행 중 오류 발생: {e}", exc_info=True)
            failed = state.get("failed_tools", []) + ["RAG"]
    ...
    async def run_code_execution_tool(
        ...
        # 질문과 관련된 코드 예제나 문서를 벡터 저장소에서 찾아 LLM에게 참고 자료로 제공합니다.
        ...
        # 검색된 컨텍스트를 바탕으로 'powerful' LLM에게 실제 실행할 코드를 생성하도록 요청합니다.
        ...
        # 이벤트 루프가 블로킹되는 것을 방지합니다. 이는 비동기 환경에서 동기 코드를 안전하게 실행하는 표준적인 방법입니다.
    ...
    async def generate_final_answer(self, state: AgentState) -> Dict[str, Any]:
        ...
        # 이를 통해 LLM은 자신이 어떤 도구를 사용했고 그 결과가 무엇인지 명확히 인지하고 답변을 생성합니다.
        context_str = ""
        ...
        if tool_choice == "RAG" and tool_outputs.get("rag_chunks"):
            ...
            # 라우터가 도구를 선택했지만 해당 도구 실행 결과가 없는 경우
            context_str = "도움말: 관련 정보를 찾지 못했거나, 선택된 도구의 결과가 없습니다."

        # 최종 프롬프트를 조립합니다. 하이브리드 컨텍스트, 도구 컨텍스트, 원본 질문 등을 모두 포함합니다.
        ...
        # LLM 스트리밍 호출을 통해 최종 답변을 생성하고, 전체 답변을 `full_answer`에 누적합니다.
        full_answer = ""
    ...
    async def output_guardrail(self, state: AgentState) -> Dict[str, Any]:
        ...
        try:
            ...
            # 오류 메시지를 포함한 안전한 답변으로 대체합니다. 이는 'fail-safe' 메커니즘입니다.
