...
class User(Base):
    ...
    user_id: Mapped[int] = mapped_column(
        BIGINT, Identity(), primary_key=True, comment="사용자 고유 ID (자동 증가)"
    )
    username: Mapped[str] = mapped_column(
        String(100), unique=True, index=True, nullable=False, comment="사용자 이름 (로그인 시 사용)"
    )
    hashed_password: Mapped[str] = mapped_column(Text, nullable=False, comment="해시된 비밀번호")
    permission_groups: Mapped[List[str]] = mapped_column(
        ARRAY(Text), server_default=func.text("ARRAY['all_users']"), nullable=False,
        comment="사용자가 속한 권한 그룹 목록 (RAG 문서 접근 제어에 사용)"
    )
    created_at: Mapped[datetime.datetime] = mapped_column(
        TIMESTAMP(timezone=True), server_default=func.current_timestamp(), comment="계정 생성일 (UTC)"
    )
    is_active: Mapped[bool] = mapped_column(
        BOOLEAN, server_default=func.text("true"), nullable=False, comment="계정 활성화 여부"
    )
...
class Document(Base):
    ...
    doc_id: Mapped[str] = mapped_column(Text, primary_key=True, comment="문서의 고유 식별자 (예: 파일 경로, URL)")
    source_type: Mapped[str] = mapped_column(String(50), nullable=True, comment="문서 출처 유형 (예: 'file', 'github')")
    owner_user_id: Mapped[int] = mapped_column(ForeignKey("users.user_id"), nullable=True, comment="문서를 업로드한 사용자 ID")
    permission_groups: Mapped[List[str]] = mapped_column(
        ARRAY(Text), nullable=False, comment="이 문서에 접근할 수 있는 권한 그룹 목록"
    )
    metadata: Mapped[Dict[str, any]] = mapped_column(JSONB, nullable=True, comment="추가적인 메타데이터 (JSONB)")
    created_at: Mapped[datetime.datetime] = mapped_column(
        TIMESTAMP(timezone=True), server_default=func.current_timestamp(), comment="문서 생성일 (UTC)"
    )
...
class DocumentChunk(Base):
    ...
    chunk_id: Mapped[int] = mapped_column(BIGINT, Identity(), primary_key=True, comment="청크의 고유 ID (자동 증가)")
    ...
    chunk_text: Mapped[str] = mapped_column(Text, nullable=False, comment="분할된 문서 조각의 텍스트 내용")
    embedding: Mapped[List[float]] = mapped_column("embedding", Text, nullable=False, comment="텍스트에 대한 벡터 임베딩 (pgvector 타입)")
    metadata: Mapped[Dict[str, any]] = mapped_column(JSONB, nullable=True, comment="청크 관련 추가 메타데이터 (JSONB)")
...
class ChatHistory(Base):
    ...
    message_id: Mapped[int] = mapped_column(BIGINT, Identity(), primary_key=True, comment="메시지 고유 ID (자동 증가)")
    user_id: Mapped[int] = mapped_column(
        ForeignKey("users.user_id", ondelete="CASCADE"), index=True, nullable=False,
        comment="메시지를 작성한 사용자의 ID. User 테이블의 외래 키."
    )
    session_id: Mapped[str] = mapped_column(Text, index=True, nullable=True, comment="채팅 세션 ID. 동일한 대화를 그룹화합니다.")
    role: Mapped[str] = mapped_column(String(20), nullable=False, comment="메시지 작성자 역할 ('user' 또는 'assistant')")
    content: Mapped[str] = mapped_column(Text, nullable=False, comment="메시지 내용")
    created_at: Mapped[datetime.datetime] = mapped_column(
        TIMESTAMP(timezone=True), server_default=func.current_timestamp(), comment="메시지 생성 시간 (UTC)"
    )
...
class UserProfile(Base):
    ...
    profile_text: Mapped[str] = mapped_column(Text, nullable=True, comment="사용자 프로필 내용 (예: '나는 파이썬 개발자입니다.')")
...
class AgentAuditLog(Base):
    ...
    log_id: Mapped[int] = mapped_column(BIGINT, Identity(), primary_key=True, comment="로그 고유 ID (자동 증가)")
    session_id: Mapped[str] = mapped_column(Text, index=True, nullable=True, comment="관련된 채팅 세션 ID")
    created_at: Mapped[datetime.datetime] = mapped_column(
        TIMESTAMP(timezone=True), server_default=func.current_timestamp(), comment="로그 생성 시간 (UTC)"
    )
    # --- 에이전트의 주요 입력값 ---
    question: Mapped[str] = mapped_column(Text, nullable=False, comment="사용자의 원본 질문")
    permission_groups: Mapped[List[str]] = mapped_column(ARRAY(Text), nullable=True, comment="요청 시점의 사용자 권한 그룹")
    # --- 에이전트의 중간 결정 및 결과 ---
    tool_choice: Mapped[str] = mapped_column(String(100), nullable=True, comment="라우터가 결정한 도구 이름")
    code_input: Mapped[str] = mapped_column(Text, nullable=True, comment="코드 실행 도구가 생성/사용한 코드")
    final_answer: Mapped[str] = mapped_column(Text, nullable=True, comment="에이전트가 생성한 최종 답변")
    # --- 전체 상태 (디버깅용) ---
    full_agent_state: Mapped[Dict[str, any]] = mapped_column(
        JSONB, server_default=func.text("'{}'::jsonb"),
