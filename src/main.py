...
from .tasks import process_document_indexing
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        logger.info("--- [FastAPI Lifespan] 모든 컴포넌트 및 AgentBrain 초기화 완료 ---")
        yield
...
class QueryRequest(BaseModel):
    ...
    permission_groups: List[str] = Field(default=["all_users"], description="사용자의 권한 그룹")
...
async def stream_agent_response(
    agent_brain: AgentBrain, inputs: Dict[str, Any]
...
async def query_corporate_core(request: Request, body: QueryRequest):
    ...
    if not agent_brain:
        raise HTTPException(status_code=500, detail="서버 컴포넌트(AgentBrain)가 로드되지 않았습니다.")
    ...
    return StreamingResponse(
        stream_agent_response(agent_brain, inputs),
