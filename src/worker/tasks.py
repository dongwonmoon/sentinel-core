...
CODE_LANGUAGE_MAP = {
    ...
    ".java": Language.JAVA, ".go": Language.GO, ".c": Language.C,
    ".cpp": Language.CPP, ".h": Language.C, ".md": Language.MARKDOWN,
}
...
async def _generate_hypothetical_question(chunk_text: str, llm: BaseLLM) -> str:
    ...
    try:
        ...
        logger.warning(f"HyDE: 가상 질문 생성 실패 (원본: '%.20s...'): %s", chunk_text, e)
        return chunk_text # 실패 시 원본 텍스트 반환

async def _load_and_split_documents(
    temp_file_path: str, file_name: str, text_splitter_default: RecursiveCharacterTextSplitter, llm: BaseLLM
) -> List[Document]:
    ...
    if language and language != Language.MARKDOWN:
        try:
            ...
            logger.warning(f"'{language.value}'용 코드 스플리터 사용 실패. 기본 스플리터로 대체합니다.")
    ...
    tasks = [_generate_hypothetical_question(chunk.page_content, llm) for chunk in split_chunks]
...
def process_document_indexing(self, file_content: bytes, file_name: str, permission_groups: List[str], owner_user_id: int):
    ...
    try:
        ...
        if file_name.lower().endswith(".zip"):
            logger.info(f"'{file_name}'은 ZIP 파일입니다. 압축 해제 및 내부 파일 처리를 시작합니다.")
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                for root, _, files in os.walk(temp_dir):
                    for file in files:
                        ...
                        try:
                            chunks = asyncio.run(
                                _load_and_split_documents(file_path, relative_path, text_splitter_default, fast_llm)
                            )
                            base_doc_id_prefix = f"file-upload-{file_name.rsplit('.zip', 1)[0]}"
                            doc_id = f"{base_doc_id_prefix}/{relative_path}"
                            for chunk in chunks:
                                chunk.metadata.update({
                                    "doc_id": doc_id, "source_type": "file-upload-zip",
                                    "original_zip": file_name, "source": relative_path
                                })
                            all_chunks_to_index.extend(chunks)
                            total_files_processed += 1
                        except Exception as e:
                            logger.warning(f"ZIP 내 파일 '{relative_path}' 처리 중 오류 발생: {e}")
        else:
            # 단일 파일 처리
            ...
            try:
                chunks = asyncio.run(
                    _load_and_split_documents(temp_file_path, file_name, text_splitter_default, fast_llm)
                )
                doc_id = f"file-upload-{file_name}"
                for chunk in chunks:
                    chunk.metadata.update({"doc_id": doc_id, "source_type": "file-upload", "source": file_name})
                all_chunks_to_index.extend(chunks)
                total_files_processed = 1
            finally:
        ...
        if not all_chunks_to_index:
            logger.warning(f"'{file_name}' 처리 후 인덱싱할 청크가 없습니다. 파일이 비어있거나 지원하지 않는 형식일 수 있습니다.")
            # 오류로 처리하지 않고, 성공적으로 처리했으나 내용이 없음을 알림
            return {"status": "warning", "message": "No content could be indexed from the file(s)."}
        ...
        documents_data = [
            {
                ...
                "embedding": chunk.metadata["embedding_source_text"], # 실제 임베딩은 vector_store에서 처리
        ...
        logger.info(f"--- [Celery Task ID: {task_id}] 인덱싱 성공: {success_message} ---")
        return {"status": "success", "message": success_message}

    except Exception as e:
        logger.error(f"--- [Celery Task ID: {task_id}] '{file_name}' 인덱싱 중 심각한 오류 발생: {e} ---", exc_info=True)
...
def process_github_repo_indexing(self, repo_url: str, permission_groups: List[str], owner_user_id: int):
    """GitHub 저장소를 클론하고, 내부 파일들을 인덱싱하는 Celery 작업입니다."""
    task_id = self.request.id
    repo_name = repo_url.split("/")[-1].replace(".git", "")
    logger.info(f"--- [Celery Task ID: {task_id}] GitHub 리포지토리 '{repo_name}' 인덱싱 시작 ---")
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            Repo.clone_from(repo_url, temp_dir, depth=50) # 최근 50개 커밋만 가져와서 클론 속도 향상
            ...
            for root, _, files in os.walk(temp_dir):
                ...
                for file in files:
                    ...
                    try:
                        ...
                        for chunk in chunks:
                            chunk.metadata.update({
                                "doc_id": doc_id, "source_type": "github-repo",
                                "repo_url": repo_url, "repo_name": repo_name, "source": relative_path
                            })
                        all_chunks_to_index.extend(chunks)
                        total_files_processed += 1
                    except Exception as e:
                        logger.warning(f"GitHub 리포지토리 내 파일 '{relative_path}' 처리 중 오류: {e}")
        
        if not all_chunks_to_index:
            logger.warning(f"'{repo_name}' 리포지토리에서 인덱싱할 콘텐츠를 찾지 못했습니다.")
            return {"status": "warning", "message": "No content could be indexed from the repository."}

        documents_data = [
            {
                "doc_id": chunk.metadata["doc_id"], "chunk_text": chunk.page_content,
                "embedding": chunk.metadata["embedding_source_text"], "metadata": chunk.metadata,
                "source_type": chunk.metadata["source_type"], "permission_groups": permission_groups,
                "owner_user_id": owner_user_id,
            } for chunk in all_chunks_to_index
        ]
        asyncio.run(vector_store.upsert_documents(documents_data=documents_data))
        ...
        logger.info(f"--- [Celery Task ID: {task_id}] GitHub 인덱싱 성공: {success_message} ---")
        return {"status": "success", "message": success_message}

    except GitCommandError as e:
        logger.error(f"--- [Celery Task ID: {task_id}] Git 클론 실패: {repo_url}. 오류: {e} ---", exc_info=True)
        # Git 오류는 재시도해도 해결되지 않을 가능성이 높으므로, 재시도하지 않고 바로 실패 처리
        return {"status": "error", "message": f"Failed to clone repository: {repo_url}. Please check the URL and permissions."}
    except Exception as e:
        logger.error(f"--- [Celery Task ID: {task_id}] '{repo_name}' 인덱싱 중 심각한 오류 발생: {e} ---", exc_info=True)
        raise self.retry(exc=e)
...
def check_stale_documents(days_old: int = 180):
    ...
    logger.info(f"[Beat Task] {days_old}일 이상 검증되지 않은 오래된 문서 스캔을 시작합니다...")
    
    find_stale_stmt = text(f"""
    ...
    """)
    insert_notification_stmt = text("INSERT INTO user_notifications (user_id, message) VALUES (:user_id, :message)")
    ...
    try:
        with get_sync_db_session() as session:
            ...
            logger.warning(f"[Beat Task] {len(stale_documents)}개의 오래된 문서를 발견했습니다. 알림 생성을 시작합니다.")
            notifications_to_create = []
            for doc in stale_documents:
                doc_dict = doc._asdict()
                doc_name = (doc_dict.get("metadata", {}) or {}).get("original_zip") or doc_dict.get("doc_id")
...
def run_scheduled_github_summary(self, task_id: int, user_id: int, repo_url: str):
    ...
    logger.info(f"[Sched Task / Celery ID: {task_id_celery}] DB Task {task_id} (사용자: {user_id}) '{repo_url}' 요약 시작...")
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            if not commits:
                logger.info(f"[Sched Task] DB Task {task_id}: '{repo_url}'에 새로운 커밋이 없습니다.")
        ...
        response = asyncio.run(fast_llm.invoke([HumanMessage(content=prompt)], config={}))
        ...
        with get_sync_db_session() as session:
            session.execute(
                text("INSERT INTO user_notifications (user_id, message) VALUES (:user_id, :message)"),
                {"user_id": user_id, "message": notification_message}
            )

        logger.info(f"[Sched Task] DB Task {task_id}: '{repo_name}' 요약 및 알림 생성을 완료했습니다.")
        return f"Summary created for {repo_name}."

    except GitCommandError as e:
        logger.error(f"[Sched Task] DB Task {task_id}: Git 클론 실패 '{repo_url}'. 오류: {e}")
        return f"Git clone failed for {repo_url}." # 재시도하지 않음
    except Exception as e:
        logger.error(f"[Sched Task] DB Task {task_id}: 요약 작업 중 오류 발생: {e}", exc_info=True)
        raise self.retry(exc=e)
...
def check_and_run_user_tasks():
    ...
    select_tasks_stmt = text("SELECT task_id, user_id, task_name, schedule, task_kwargs FROM scheduled_tasks WHERE is_active = true")
    now = datetime.now()
    tasks_dispatched = 0
    ...
    try:
        with get_sync_db_session() as session:
            ...
            for task in active_tasks:
                ...
                logger.info(f"[Beat Task] DB Task {task_id} ('{task_name}')의 실행 시간이 되었습니다. 큐에 작업을 발행합니다.")
                
                if task_name == "run_scheduled_github_summary":
                    kwargs = task_dict.get("task_kwargs", {})
                    repo_url = kwargs.get("repo_url")
                    if repo_url:
                        run_scheduled_github_summary.delay(task_id=task_id, user_id=task_dict["user_id"], repo_url=repo_url)
                        tasks_dispatched += 1
                # 여기에 다른 사용자 정의 태스크 핸들러를 추가할 수 있습니다.
    except Exception as e:
        logger.error(f"[Beat Task] 사용자 정의 스케줄 스캔 중 심각한 오류 발생: {e}", exc_info=True)
        return f"Error during user task scan: {e}"

    if tasks_dispatched > 0:
        logger.info(f"[Beat Task] 스캔 완료. {tasks_dispatched}개의 사용자 작업을 큐에 발행했습니다.")
